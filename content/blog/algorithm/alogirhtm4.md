...
1. **해답 검사(Solution Check)** 새로운 집합이 problem에 대한 solution인지 판단한다.  
...
예를들어, 도시끼리 모두 연결되게 하면서 도로의 길이가 최소가 되도록 도시를 설계하거나, 통신선의 길이가 최소가 되도록 케이블 망을 구성하는 문제 등 네트워크 디자인과 같은 분야 등에서 활용할 수 있다.

<br>

이제 Tree가 주어졌을 때, MST를 구하는 알고리즘에 대해 생각해보자.

만약 Brute-Force 방법으로 모든 spanning tree를 구하고 비용이 최소인 tree를 선택한다면, 최악의 경우 지수시간보다 나쁘다. ($$\Theta (n)$$)  
따라서 더 효율적인 알고리즘을 생각해야 한다.

### Prim's Algorithm

프림 알고리즘은 Tree $$G = (V, E)$$ 가 주어졌을 때, (V = Vertices, E = Edges)
...
1. 매 순간 주어진 $$V - Y$$의 정점 중에서 가장 최소거리인 정점을 선택한다.
...
하지만 이것이 **minimum** spanning tree인지는 어떻게 확신할 수 있을까?

##### Promissing

무향 그래프 $$G = (V, E)$$가 주어졌다고 하자. $$E$$의 부분집합 $$F$$에 간선을 추가해 spanning tree를 만들 수 있으면, $$F$$는 `유망하다(promissing)` 라고 한다.

이제 반복문에서 간선 $$e$$를 추가한 집합 $$F$$가 항상 promissing인것을 증명하면 된다.

- $$F$$가 promissing하면, MST인 $$(V, F')$$에 대해서 $$F \subseteq F'$$이다.
...
- 그러므로 $$F \cup \{e\}$$ 역시 promissing하다.
- 귀납법에 의해, 반복문의 최종 단계를 포함한 모든 `F는 promissing하다.`
...
크루스칼 알고리즘은 Tree $$G = (V, E)$$ 가 주어졌을 때, (V = Vertices, E = Edges)
